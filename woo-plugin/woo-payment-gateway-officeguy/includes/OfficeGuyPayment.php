...
class OfficeGuyPayment
{
    ...
    public static function ValidateOrderFields($Gateway)
    {
        ...
        if (OfficeGuyRequestHelpers::Post('wc-' . $Gateway->id . '-payment-token') && OfficeGuyRequestHelpers::Post('wc-' . $Gateway->id . '-payment-token') !== 'new') {
            ...
            if ($Gateway->settings['pci'] == 'no') {
                ...
                if ($Gateway->settings['cvv'] == 'yes' && ! ctype_digit($CVV)) { // Check security code
    ...
    public static function ProcessOrder($Gateway, $Order, $IsWooCommerceSubscriptionPayment)
    {
        ...
        [$Request, $Token] = OfficeGuyPayment::GetOrderRequest($Gateway, $Order, $ItemMethods, $PaymentsCount, $IsWooCommerceSubscriptionPayment);
        ...
        } elseif (in_array('subscription', $ItemMethods)) {
        ...
        } elseif ($Gateway->settings['pci'] == 'redirect') {
    ...
    public static function ProcessBitOrder($Gateway, $Order)
    {
        ...
        [$Request, $Token] = OfficeGuyPayment::GetOrderRequest($OfficeGuyGateway, $Order, $ItemMethods, 1, false);
    ...
    public static function GetOrderItemMethods($Order)
    {
        ...
        foreach ($Order->get_items() as $OrderItem) {
            ...
            } elseif (! in_array('simple', $ItemMethods)) {
    ...
    public static function GetOrderLanguage($Gateway)
    {
        ...
        if ($Gateway->settings['automaticlanguages'] == 'yes') {
            ...
            if ($locale == 'en_US' || $locale == 'en') {
                $Language = 'English';
            } elseif ($locale == 'ar_AR' || $locale == 'ar') {
                $Language = 'Arabic';
            } elseif ($locale == 'es_ES' || $locale == 'es') {
                $Language = 'Spanish';
            } elseif ($locale == 'he_IL' || $locale == 'he') {
                $Language = 'Hebrew';
            }
        }
    ...
    public static function CreateDocumentOnPaymentComplete($OrderID)
    {
        ...
        if ($UseCron) {
            if (! wp_schedule_single_event(time() + 5, 'officeguy_documentonpayment', [$OrderID])) {
    ...
    {
        return in_array(get_woocommerce_currency(), ['ILS', 'USD', 'EUR', 'CAD', 'GBP', 'CHF', 'AUD', 'JPY', 'SEK', 'NOK', 'DKK', 'ZAR', 'JOD', 'LBP', 'EGP', 'BGN', 'CZK', 'HUF', 'PLN', 'RON', 'ISK', 'HRK', 'RUB', 'TRY', 'BRL', 'CNY', 'HKD', 'IDR', 'INR', 'KRW', 'MXN', 'MYR', 'NZD', 'PHP', 'SGD', 'THB']);
    }
